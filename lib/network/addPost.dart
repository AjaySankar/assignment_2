...
// Key for a offline post
const String NEW_POST_SHARED_PREF_KEY_PREFIX = "NewInstaPost-";
// Key for the list of offline post ids
...
class AddInstaPost extends InstaPostRequest {
  ...
  int generateNewInstaPostId() {
    // Post id to identify a offline stored post
  ...
  // Saves new post to shared preferences when device is offline
  ...
  // Uploads a post when device goes  online. 'postId' is the offline post id.
  // Returns if post upload is successful.
  Future<bool> uploadOfflinePost(int postId) async {
    // Get saved post from shared preferences
  ...
  // Delete offline post data of successfully uploaded posts.
  ...
  // Update the offline posts ids list with failed posts id. Maybe try some other time..
  ...
  // Processes offline posts saved in the device.
